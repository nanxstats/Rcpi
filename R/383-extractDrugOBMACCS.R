...
extractDrugOBMACCS = function (molecules, type = c('smile', 'sdf')) {
  ...
  if ( !ChemmineOB.exist ) stop('The ChemmineOB package is required to run extractDrugOBMACCS(). Please follow the instructions on http://www.bioconductor.org/packages/release/bioc/html/ChemmineOB.html to install it.')
  ...
  if (type == 'smile') {
    ...
    if ( length(molecules) == 1L ) {
      ...
      for ( i in 1:length(molecules) ) fp[i, ] = ChemmineOB::fingerprint_OB('SMI', molecules[i], 'MACCS')
    ...
    if ( length(smiclean) == 1L ) {
      ...
      for ( i in 1:length(smiclean) ) fp[i, ] = ChemmineOB::fingerprint_OB('SMI', smiclean[i], 'MACCS')
