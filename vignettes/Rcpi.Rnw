...
\usepackage{amssymb}
\usepackage{amsmath}
%\VignetteIndexEntry{Rcpi: R/Bioconductor Package for Predictive Modeling of Compound-Protein Interactions}
...
\title{\pkg{Rcpi}: \proglang{R}/Bioconductor Package for Predictive Modeling of Compound-Protein Interactions}
\Plaintitle{Rcpi: R/Bioconductor Package for Predictive Modeling of Compound-Protein Interactions}
\Shorttitle{\pkg{Rcpi}: R/Bioconductor Package for Predictive Modeling of Compound-Protein Interactions}
...
\SweaveOpts{concordance=TRUE}
    ...
    {\LARGE \pkg{Rcpi}\textbf{: \proglang{R}/Bioconductor Package for Predictive Modeling of Compound-Protein Interactions}}\\[1.2\baselineskip]
    ...
    \vfill
    {\Large Insert Logo here}\\
    {\scshape Computational Biology and Drug Design Group} \\
    {\scshape Central South University}\par
    \end{center}
...
The \pkg{Rcpi} package \citep{CRAN:Rcpi} presented in this vignette offers a comprehensive \proglang{R}/Bioconductor package emphasizing the integration of bioinformatics and chemoinformatics into a molecular informatics platform for drug discovery.

\pkg{Rcpi} is a powerful \proglang{R}/Bioconductor package for computing commonly used structural and physicochemical descriptors of protein and peptide sequences, molecular descriptors of drug molecules from their topology, and protein-protein interaction and protein-ligand interaction descriptors. \pkg{Rcpi} calculates six protein descriptor groups composed of ? descriptor that include ? descriptor types and ? descriptors, ? drug descriptor groups composed of ? descriptor types that include ? descriptors. In addition, \pkg{Rcpi} provides ? types of molecular fingerprint systems for drug molecules, including ?, E-state fingerprints, MACCS keys, FP4 keys, topological torsion fingerprints, and ? fingerprints. By combining different types of descriptors from drugs and proteins in different methods, interaction descriptors representing protein-protein or drug-protein interactions could be conveniently generated. These computed descriptors are widely used in various fields relevant to bioinformatics, chemoinformatics, proteochemometrics and chemogenomics.

\pkg{Rcpi} implemented and integrated the state-of-the-art protein sequence descriptors and molecular descriptors/fingerprints with \proglang{R}. Parallelized molecule and sequence retrieving from multiple websites. Parallelized similarity computation for molecules and proteins by FP/MCS and sequence alignment/GO. Several self-explanatory examples have been included to illustrate the benefits of using \pkg{Rcpi}. Many more examples are available within the package.

The \pkg{Rcpi} package is available from Bioconductor (\url{http://bioconductor.org}), visit \url{http://bioconductor.org/packages/release/bioc/html/Rcpi.html} for more details. This vignette corresponds to \pkg{Rcpi} version \Sexpr{Rcpi.version} and was typeset on \Sexpr{now.date}.

To install the \pkg{Rcpi} package, type
...
To use all functions in \pkg{Rcpi} package, we strongly recommend install the suggested packages:

\begin{CodeInput}
source('http://bioconductor.org/biocLite.R')
biocLite('Rcpi', dependencies = c('Depends', 'Suggests'))
\end{CodeInput}
...
Protein subcellular localization prediction involves the computational prediction of where a protein resides in a cell. Since determining the subcellular localization of a protein experimentally is a time consuming task, this is a solution. It is an important component of bioinformatics-based prediction of protein function and genome annotation, and it can aid the identification of novel drug targets.

Here we use the subcellular localization dataset of human proteins presented in the study of \cite{chou2008cell} for demonstration. The complete dataset includes 3134 protein sequences (2750 different proteins), classified into 14 human subcellular locations. We select two classes of proteins as our benchmark dataset. Class 1 contains 325 \emph{extracell} proteins, and class 2 includes 307 \emph{mitochondrion} proteins.
...
Two protein sequences were removed from each class. For the remaining sequences, we calculate the amphiphilic pseudo amino acid composition (APAAC) descriptor \citep{chou2005using} and make class labels for classification modeling.
...
For protein sequences in \pkg{Rcpi}, the functions of commonly used descriptors and proteochemometric (PCM) modeling descriptors are named after \code{extractProt...()} and \code{extractPCM...()} in \pkg{Rcpi}.
...
With the model trained on the training set, we predict on the test set and plot the ROC curve with the \pkg{pROC} package, as shown in figure \ref{fig:ex1-1}.
...
For chemoinformatics research, \pkg{Rcpi} calculates molecular descriptors and fingerprints, computes pairwise similarities derived by fingerprints and maximum common substructure search. \pkg{Rcpi} also provides \code{searchDrug()} for parallelized molecular similarity search based on these similarity types.
...
In \cite{yan2012comparison}, a quantitative structure-retention relationship study was performed for 656 flavor compounds on four stationary phases of different polarities, using constitutional, topological and geometrical molecular descriptors. The gas chromatographic retention indices (RIs) of these compounds were accurately predicted using linear models. Here we chose the molecules and retention indices of one stationary phase (OV101) as our benchmark dataset.

Since it would be rather tedious to implement the complete cross-validation procedures, the \proglang{R} package \pkg{caret} is used here. To run the \proglang{R} code examples below, users need to install the \pkg{caret} package and the required predictive modeling packages first. The \pkg{caret} package provides a unified interface for the modeling tuning task across different statistical machine learning packages. It is particularly helpful in QSAR modeling, for it contains tools for data splitting, pre-processing, feature selection, model tuning and other functionalities.

Just like the last section, we load the \pkg{Rcpi} package, and read the molecules stored in a SMILES file with \code{readMolFromSmi()}:
...
After the molecules were properly loaded, we calculate several selected molecular descriptors. The corresponding functions are all named after \code{extractDrug...()} in \pkg{Rcpi}:
...
We could train a partial least squares model with the \pkg{pls} and \pkg{caret} package. The cross-validation setting is 5-fold repeated CV (repeat for 10 times).
...
We could see that the RMSE of the PLS regression model was decreasing when the number of principal components (\code{ncomp}) was increasing. We can plot the components and RMSE to helps us select the desired number of principal components used in the model.
...
From figure \ref{fig:ex2-1}, we could see that selecting six or seven components is acceptable. Then we plot the experimental RIs and the predicted RIs to see if the model fits well on the training set, and it does fit well:
...
In the perspective of quantitative pharmacology, the successful discovery of novel drugs depends on the pharmacokinetics properties, like \textbf{a}bsorption, \textbf{d}istribution, \textbf{m}etabolism, and \textbf{e}xcretion. In addition, the potential \textbf{t}oxicity of chemical compounds is taken into account. QSAR or QSPR methods are usually employed to predict the ADME/T qualities of compounds.

In the study of \cite{cao2012silico}, quantitative structure-toxicity relationship (QSTR) models were established for classifying five toxicity datasets from US Environmental Protection Agency (EPA) Distributed Structure-Searchable Toxicity (DSSTox) database. Here we use the maximum recommended daily dose dataset (FDAMDD) from FDA Center for Drug Evaluation and Research as our benchmark dataset.

First, we load the drug molecules stored in a SMILES file:
...
The \code{x.mol} is used for computing the MACCS and Estate fingerprints, the \code{x.smi} is used for computing the FP4 fingerprints. The 0-1 class labels stored in \code{FDAMDD.csv} indicates whether the drug molecule has high toxicity or not.
...
As the nature of fingerprint-based structure representation, the calculated 0-1 matrix \code{x1}, \code{x2}, and \code{x3} will be very sparse. Since there are several variables with nearly exactly the same value for all the observations, we should remove them with \code{nearZeroVar()} in \pkg{caret} before modeling, and split our training set and test set:
...
On the training sets, we train three classification models separately using SVM (RBF kernel), using the \pkg{kernlab} package and \pkg{caret} package. The cross-validation setting is 5-fold repeated CV (repeat for 10 times). Then print the cross-validation result.
...
The result when using E-state fingerprints:
...
The result when using MACCS keys:
...
The result when using FP4 fingerprints:
...
We will predict on the test sets with the established models, and plot the ROC curves in one figure, as is shown in figure \ref{fig:ex3-1}.
...
With the similarity matrix \code{simmat}, we will try to apply hierarchical clustering on it, then visualize the clustering result:
...
Using \code{searchDrug()}, we could do the parallelized drug molecular similarity search. Here we choose the search criterion to be MACCS keys with cosine similarity, FP2 fingerprints with tanimoto similarity, and maximum common substructure search with tanimoto similarity.
...
The search result is a numerical vector, each element's name is the molecule number in the database, and the value is the similarity value between the query molecule and it. We shall print the top search results:
...
The MCS search result is stored in a list, which contains the original MCS result provided by the \pkg{fmcsR} package \citep{wang2013fmcsr}, the Tanimoto coefficient and overlap coefficient.
...
By using \code{calcDrugMCSSim(..., plot = TRUE)}, the maximum common substructure is presented in figure \ref{fig:ex5-1}.
...
For chemogenomics modeling, \pkg{Rcpi} calculates compound-protein interaction descriptors and protein-protein interaction descriptors.
...
The prediction of novel interactions between drugs and target proteins is a key area in genomic drug discovery. In this example, we use the G protein-coupled receptor (GPCR) dataset provided in \cite{yamanishi2008prediction} as our benchmark dataset.

Generally speaking, a drug-target interaction network can be naturally modeled as a bipartite graph, where the nodes are target proteins or drug molecules and edges (only drugs and proteins could be connected by edges) represent drug-target interactions. Initially, the graph only contains edges describing the \emph{real} drug-target interactions determined by experiments or other ways. In this example, all real drug-target interaction pairs (i.e., 635 drug-target interactions) are used as the positive samples. For negative examples we select random, non-interacting pairs from these drugs and proteins. They are constructed as follows: 
...
The result is shown in figure \ref{fig:ex6-1}.
...
We will download the target protein sequences (in FASTA format) and drug molecule (in SMILES format) from the KEGG website in parallel:
...
If the connection is slow or accidentally interrupts, just try more times.

The functions in \pkg{Rcpi} named after \code{getMolFrom...()}, \code{getSmiFrom...()} supports the parallelized retrieval of (drug) molecules from PubChem, ChEMBL, CAS, KEGG, and DrugBank. The functions named after \code{getSeqFrom...()}, \code{getFASTAFrom...()}, \code{getPDBFrom...()} supports the parallelized retrieval of proteins from UniProt, KEGG and RCSB PDB. The functions \code{getDrug()} and \code{getProt()} are two integrated wrapper functions for downloading the molecules and protein sequences from these website.

After the sequences are downloaded, we could calculate the protein descriptors and molecular descriptors:
...
Since the descriptors is for the \emph{uniqued} drug and protein list, we need to generate the full descriptor matrix for the training data:
...
Generate drug-protein interaction descriptors using \code{getCPI()}.
...
For interaction protein A and protein B, let $d_1^{1 \times p}$ and $d_2^{1 \times p}$ be the descriptor vectors. There are three methods to construct the protein-protein interaction descriptor vector $d$:
